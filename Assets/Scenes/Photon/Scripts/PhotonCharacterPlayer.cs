...
namespace Photon
{
    public class PhotonCharacterPlayer : MonoBehaviourPunCallbacks
    {
        ...
        private void Start()
        {
            ...
            // --- local-first username + network sync ---
            if (photonView.IsMine)
            {
                // Local-only set so your client immediately shows the right name
                string displayName = PhotonNetwork.LocalPlayer.NickName; // or PhotonNetwork.NickName
                username.text = displayName;

                // Sync that name to other clients (OthersBuffered so new joiners also get it)
                photonView.RPC(nameof(RPC_SetUsername), Pun.RpcTarget.OthersBuffered, displayName);
            }
        }
        [PunRPC]
        private void RPC_SetUsername(string name)
        {
            // This runs on remote instances; just set the UI text on that prefab instance
            username.text = name;
        }
        private void CheckCam()
        {
            if (!photonView.IsMine) return;
            vcam = GameObject.Find("FreeLook Camera").GetComponent<CinemachineCamera>();
